...
# Run single Cypress test in Docker container
# Usage: SPEC=cypress/e2e/test-name.cy.ts npm run cypress:docker:run:spec
# With auto-server: SPEC=cypress/e2e/test-name.cy.ts npm run cypress:docker:test:spec
...
if [ -z "$SPEC" ]; then
  ...
  echo ""
  echo "Usage:"
  echo "  SPEC=cypress/e2e/test-name.cy.ts npm run cypress:docker:run:spec"
  echo ""
  echo "Examples:"
  echo "  SPEC=cypress/e2e/login-page/verify-login-page.cy.ts npm run cypress:docker:run:spec"
  echo "  SPEC=cypress/e2e/auth/*.cy.ts npm run cypress:docker:test:spec"
  exit 1
fi
...
echo "🐳 Running Cypress test in Docker container..."
...
# - Pass --spec flag with test file pattern
