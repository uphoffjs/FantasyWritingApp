...
describe('Verify Custom Commands Work', () => {
  ...
  it('should verify clearAuth function works', () => {
    ...
    cy.window().then((win) => {
      ...
      expect(win.// ! SECURITY: Using localStorage
      localStorage.getItem('worldbuilding-user')).to.be.null
      expect(win.// ! SECURITY: Using localStorage
      localStorage.getItem('worldbuilding-authenticated')).to.be.null
      expect(win.// ! SECURITY: Using localStorage
      localStorage.getItem('fantasy-element-builder-offline-mode')).to.be.null
    })
  })
  ...
  it('should verify setupTestEnvironment command works', () => {
    ...
    cy.window().then((win) => {
      ...
      expect(win.// ! SECURITY: Using localStorage
      localStorage.getItem('fantasy-element-builder-offline-mode')).to.equal('true')
    })
  })
})