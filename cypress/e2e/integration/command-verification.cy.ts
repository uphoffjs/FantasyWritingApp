...
describe('Verify Custom Commands Work', () => {
  ...
  it('should verify setupAuth function works', () => {
    ...
    cy.window().then((win) => {
      ...
      const userData = win.localStorage.getItem('writing-app-user')
      expect(userData).to.not.be.null
      
      const isAuthenticated = win.localStorage.getItem('writing-app-authenticated')
      expect(isAuthenticated).to.equal('true')
      
      // Verify offline mode was set
      const offlineMode = win.localStorage.getItem('fantasy-writing-app-offline-mode')
  ...
  it('should verify clearAuth function works', () => {
    ...
    cy.window().then((win) => {
      ...
      expect(win.localStorage.getItem('writing-app-user')).to.be.null
      expect(win.localStorage.getItem('writing-app-authenticated')).to.be.null
      expect(win.localStorage.getItem('fantasy-writing-app-offline-mode')).to.be.null
    })
  })
  ...
  it('should verify story-specific storage keys work', () => {
    ...
    cy.window().then((win) => {
      ...
      // Verify keys exist
      expect(win.localStorage.getItem('current-story-id')).to.equal('test-story-123')
      expect(win.localStorage.getItem('story-draft-auto-save')).to.equal('true')
    })
    
    // Clear and verify cleanup
    cy.clearLocalStorage()
    cy.window().then((win) => {
      expect(win.localStorage.getItem('current-story-id')).to.be.null
      expect(win.localStorage.getItem('story-draft-auto-save')).to.be.null
    })
  })
})